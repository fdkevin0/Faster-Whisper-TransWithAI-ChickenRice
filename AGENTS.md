...
* 你正在协助的对象是 **FDKevin**。
* 假设 FDKevin 是一名经验丰富的资深后端 / 数据库工程师，熟悉 Rust、Go、Python 等主流语言及其生态。
* FDKevin 重视“Slow is Fast”，关注点在于：推理质量、抽象与架构、长期可维护性，而不是短期速度。
* 你的核心目标：
...
Core Python code lives in `src/faster_whisper_transwithai_chickenrice/` (`infer.py`, VAD integration, i18n helpers).  
Root entrypoints are `infer.py` (local wrapper), `modal_infer.py` (Modal cloud workflow), and `download_models.py` (model bootstrap).  
Packaging/build artifacts are managed by `project.spec`, `modal.spec`, `build_windows.py`, and `runtime_hook.py`.  
Environment definitions are in `environment-cuda118.yml`, `environment-cuda122.yml`, `environment-cuda128.yml`, and `environment-modal.yml`.  
Localized strings are in `locales/en-US/messages.json` and `locales/zh-CN/messages.json`.

## Build, Test, and Development Commands
- `conda env create -f environment-cuda118.yml` (or `cuda122/cuda128`): create the main dev/runtime environment.
- `conda activate faster-whisper-cu118`: activate the environment.
- `python infer.py --help`: inspect local CLI options.
- `python infer.py --device cuda <path-or-dir>`: run transcription/translation on local files.
- `python modal_infer.py`: run interactive Modal cloud inference flow.
- `python build_windows.py`: build distributable executables via PyInstaller specs.
- `pytest`: run tests if/when test files are present.

## Coding Style & Naming Conventions
Use Python 3.10-compatible code, 4-space indentation, and UTF-8 file encoding.  
Follow existing style: snake_case for functions/variables, PascalCase for dataclasses/classes, and explicit type hints where practical.  
Keep CLI flags descriptive and long-form (example: `--model_name_or_path`, `--merge_max_gap_ms`).  
Preserve bilingual user-facing text conventions already used in docs and prompts.

## Testing Guidelines
`pytest` and `pytest-cov` are included in CUDA environment files. Add tests under `tests/` using `test_*.py` naming.  
Prioritize coverage for argument parsing, segment merge behavior, device/compute-type selection, and VAD injection paths.  
Before opening a PR, run `pytest` locally and include any manual validation steps for GPU/Modal flows.

## Commit & Pull Request Guidelines
Recent history follows Conventional Commit-like prefixes: `feat:`, `fix:`, `enh:`. Keep this format and scope each commit to one change.  
PRs should include:
- A clear summary of behavior changes
- Linked issue(s) when applicable
- Repro/validation commands used (for example `python infer.py ...`, `python build_windows.py`)
- Screenshots or logs for UI/packaging changes when relevant
